...
class NewsCategoryController extends Controller
{
	...
	public function index()
	{
		...
		$lists = NewsCategory::whereRaw($where)
		...
		foreach($column_array as $key => $value)
		{
			...
			$sorting_url = 'news_category?'.($search!="" ? 'search='.$search.'&' : '').'orderby='.$sorting_url_orderby.'&order='.$sorting_url_order;

			$sorting_array[$key] = array('sorting_class' => $sorting_class, 'sorting_url' => $sorting_url);
		}

		return view('admin.news_category.index', compact('lists','column_array','sorting_array','search'));
	}

	public function add()
	{
		return view('admin.news_category.add');
	}
	...
	public function insert(Request $request)
	{
		...
		$rules = array(
			...
			'slug' => 'required|string|max:255|unique:news_category',
			'rank' => 'required|int',
			'status' => 'required|int'
		);
		...
		if ($validator->fails())
		{
			return Redirect::to(Admin_Prefix.'category/add')->withErrors($validator)->withInput(); 
		}
		else
		{
			try {
			...
			return redirect()->back()->with('success', 'News Category has been added successfully.');
	...
	{
		$list = NewsCategory::where('id',$id)->first(); 
		if (!$list) {
			return redirect()->to('newsCategory')->with('error', 'Opps!! sorry!! problem occurred.Please try again!');
		}
		return view('admin.news_category.edit', compact('list'));
	}
	...
	public function update(Request $request)
	{
		...
		$rules = array(
			'name' => 'required|string|max:255',
			'slug' => 'required|string|max:255|unique:news_category,slug,'.$id,
			'rank' => 'required|int',
			'status' => 'required|int',
		);
		if($request->hasfile('image'))
		...
		if ($validator->fails())
		{
			return Redirect::to(Admin_Prefix.'newsCategory/edit/'.$id)->withErrors($validator)->withInput(); 
		}
		else
		{
			try {
			...
			return redirect()->back()->with('success', 'News Category has been updated successfully.');
	...
	public function delete($id)
	{
		...
		return redirect()->back()->with('success', 'News Category has been deleted successfully.');
	}
