    ...
    public function siteMap()
{
    ...
    $pages = Page::whereIn('posttype', [
        'blog', 'page', 'service', 'pricing', 'seo', 'city-service', 'city-business-service', 'business-service'
    ])->get();
    ...
    foreach ($pages as $page) {
        $url = '';
        $lastmod = $page->updated_at ? $page->updated_at->tz('UTC')->toAtomString() : '';
        ...
        switch ($page->posttype) {
            ...
            case 'city-service':
                foreach ($cities as $cityVal) {
                    if ($cityVal && $page->service_parent_id) {
                        $pagesData = Page::where('id', $page->service_parent_id)->first();
                        if ($pagesData && $cityVal->slug && $pagesData->slug) {
            ...
            case 'business-service':
                foreach ($business as $businessVal) {
                    if ($businessVal && $page->service_parent_id) {
                        $pagesData = Page::where('id', $page->service_parent_id)->first();
                        if ($pagesData && $pagesData->slug && $businessVal->slug) {
            ...
            case 'city-business-service':
                foreach ($cities as $cityVal) {
                    foreach ($business as $businessVal) {
                        if ($cityVal && $businessVal && $page->service_parent_id) {
                            $pagesData = Page::where('id', $page->service_parent_id)->first();
                            if ($pagesData && $cityVal->slug && $pagesData->slug && $businessVal->slug) {
