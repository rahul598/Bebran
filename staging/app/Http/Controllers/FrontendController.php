        ...
        if ($request->isMethod('post')) {
            $data = $request->all();
                ...
                else
                {
                        ...
                        $obj->guest_mobile = $request->number_new;
    ...
    public function dynamicCityView(Request $request, $city, $slug){
        ...
        if ($cityExists) {
            ...
            if($childPage){
                ...
                {
                    @$setting[0]->value = str_replace(['{city}', '{city1}'], [$getCityName, ucfirst($getCityName)], $childPage->meta_title);
                }
                $pageMetaTag = '';
...

                {
                    ...
                    $pageMetaTag = str_replace(['{city}', '{city1}'], [$getCityName, ucfirst($getCityName)], $childPage->meta_tag);
    ...
    public function showCart()
{
    ...
    // Initialize $cartItems to avoid undefined variable errors
    $cartItems = collect(); // Using an empty collection
    
    if ($clientData && isset($clientData->id)) {
        // Fetch cart items if client data exists
        $cartItems = DB::table('cart_items')->where('client_id', $clientData->id)->get();
    }

    // Convert cart items to array, even if it's an empty collection
    ...
    if (!empty($client_data)) {
        foreach ($client_data as $key => $val) {
            ...
            $arr[] = [
                ...
                'service_addon_price' => $service_addon->discount_price ?? 0, // Handle null case
                'service_addon_id' => $service_addon->service_type ?? null,
                'cart_id' => $val->id,
            ];

            $price[] = $package->discount_price;
            $addon[] = $service_addon->discount_price ?? 0; // Handle null case
        }
    }
	...
	public function value_total(Request $request){   
            ...
            // Forget the 'total_amount' session key
            ...
            // Fetch the addon service price if value3 is provided
