...
.packages-box.package-active{
    padding-left:0;
}
.packages-box .btn-box .btn-primary{
    width:100%;
    text-align:center;
}
.packages-box .btn-box .btn-primary {
...
.package-active .btn-box .btn-primary{
    border: 1px solid #facc15;
    background-color: #facc15;
    color: #0f273b;    
}
.packages-box .btn-box .btn-primary:hover{
    background-color: #facc15;
    color: #0f273b;
    border: 1px solid #facc15;
}
...
.w-2{width: 23px !important;}
.w-3{width: 10% !important;}
.w-85{width: 85% !important;}
.ml-2{margin-left:6px}
.list-s-m{ padding: 20px 0px !important;margin:0 0 !important;}
.packages-box ul li {
    padding: 10px;
    margin: 0px;
    color: #323232;
...
.dropdown.packages h5 span{font-size: 34px;}
sup.currency {
    top: -15px;
    left: 13px;
    font-size: 30px;
...
.dropdown.packages h5 del{font-size: 15px;}
.sh{box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);}
.rd-1 {
...
.f-14{font-size:14px}
.f-17{font-size:17px}.f-b{font-weight: 600;}
.btw-1 {
    width: 90% !important;
    margin: auto;
    display: block;
...
.mt-6{margin-bottom: 4rem !important;} 
.mb-6{margin-top: 4rem !important;}
.mh-670{min-height: 670px;position:relative;}

.f-n{font-weight: 500;}
.btn-c{text-transform: uppercase; font-size: 18px; font-weight: 500 !important;padding: 20px 55px;}
...
<div class="innerbanner-area">  
    <div class="macbook-air-1">
        <section class="container">
            <div class="row"> 
            <ul class="nav nav-pills justify-content-center mt-6 mb-6" id="exTab1" role="tablist">
    ...
    <div class="price_card_figma tab-content mb-5 clearfix">
        ...
        <div class="tab-pane  d-flex justify-content-between fade show active" id="a1" aria-labelledby="ex2-tab-1" role="tabpanel">
            <div class="row row-cols-1 row-cols-sm-1 row-cols-md-4 row-cols-lg-4 row-cols-xxl-4 g-0">
                ...
                <?php
                    $main_price = (float)$val['main_price'];
                    $discount_price = (float)$val['discount_price'];
                    $percentage = ($main_price > 0 && $discount_price < $main_price) ? (($main_price - $discount_price) / $main_price) * 100 : 0;
                    //$percentage = number_format($percentage, 2); // Format to 2 decimal places
                ?>
                <div class="col">
                    <div class="mh-670 packages-box {{ $val['most_popular'] != 0 ? 'package-active' : '' }}">
                        <div class="tag">{{ ucfirst($val['plan_name']) }}</div>
                        <div class="dropdown packages">
                            <button class="btn m-c">
                                <h5>
                            <div class="mb-1">
                                @if($discount_price < $main_price)
                                    <div class="d-flex justify-content-center align-items-center mb-2">
                                        <div>
                                        <del>₹{{ number_format($val['main_price']) }}</del>
                                        </div>
                                        @if($percentage > 0)
                                        <div class="off-button-wrapper ml-3">
                                            <div class="off-button">
                                                <div class="off-button-child"></div>
                                                <div class="off f-14 f-n">{{ $percentage }}% Off</div>
                                            </div>
                                        </div>
                                         @endif  
                                    </div>
                                    <div>
                                        <span><sup class="currency">₹</sup></span>
                                        <span>{{ number_format($val['discount_price']) }}</span>
                                        <sub>/ mo</sub>
                                    </div>
                                @else
                                    <span><sup class="currency">₹</sup></span>
                                    <span>{{ number_format($val['main_price']) }}</span>
                                    <sub>/ mo</sub>
                                @endif
                            </div>
                                </h5>
                        ...
                        <div class="btn-box">
                            <a href="{{ route('purchase', [
                                ...
                                'id' => $val['id']
                            ]) }}" class="btn-primary btw-1">SUBSCRIBE NOW</a>
                        </div>
                        ...
                        <ul class="list-s-m">
                            @php
                                $features = json_decode($smdes->small_features);
                            @endphp
                            @foreach($features as $fea)
                            <li>
                                <div class="d-flex align-items-center">
                                    <div class="w-3 w-3">
                                        <img class="w-2" src="https://staging.bebran.com/public/frontend2/images/check.svg" alt="Check Icon" class="list-icon">
                                    </div>
                                    <div class="ml-2 w-85">{{ $fea }}</div>
                                </div>
                            </li>
                            @endforeach
                        </ul>
                        ...
                        @endforeach
                        <div class="btn-box">
                            <a href="#feature" class="btn-primary sh rd-1 btw-2">View Details</a>
                        </div>
                    </div>
...
<div class="row row-cols-1 row-cols-sm-1 row-cols-md-4 row-cols-lg-4 row-cols-xxl-4 g-0">
    @if(!empty($data1))
        @foreach($data1 as $key1 => $val1) 
                <?php
                    $main_price = (float)$val1['main_price'];
                    $discount_price = (float)$val1['discount_price'];
                    $percentage = ($main_price > 0 && $discount_price < $main_price) ? (($main_price - $discount_price) / $main_price) * 100 : 0;
                    //$percentage = number_format($percentage, 2); // Format to 2 decimal places
                ?>        
        <div class="col">
            <div class="mh-670 packages-box {{ $val1['most_popular'] != 0 ? 'package-active' : '' }}">
                <div class="tag">{{ ucfirst($val1['plan_name']) }}</div>
                <div class="dropdown packages">
                    <button class="btn m-c">
                        <h5>
                            <div class="mb-1">
                                @if($discount_price < $main_price)
                                    <div class="d-flex justify-content-center align-items-center mb-2">
                                        <div>
                                        <del>₹{{ number_format($val1['main_price']) }}</del>
                                        </div>
                                        @if($percentage > 0)
                                        <div class="off-button-wrapper ml-3">
                                            <div class="off-button">
                                    ...
                                    <div>
                                        <span><sup class="currency">₹</sup></span>
                                        <span>{{ number_format($val1['discount_price']) }}</span>
                                        <sub>/ mo</sub>
                                    </div>
                                @else
                                    <span><sup class="currency">₹</sup></span>
                                    <span>{{ number_format($val1['main_price']) }}</span>
                                    <sub>/ mo</sub>
                                @endif
                            </div>
                        </h5>
                    ...
                    <ul class="list-s-m">
                        ...
                        @foreach($features as $fea)
                            <li>
                                <div class="d-flex align-items-center">
                                    <div class="w-3 w-3">
                                        <img class="w-2" src="https://staging.bebran.com/public/frontend2/images/check.svg" alt="Check Icon">
                                    </div>
                                    <div class="ml-2 w-85">{{ $fea }}</div>
                                </div>
                            </li>
...
<div class="row row-cols-1 row-cols-sm-1 row-cols-md-4 row-cols-lg-4 row-cols-xxl-4 g-0">
    @if(!empty($data2))
        @foreach($data2 as $key2 => $val2) 
                <?php
                    $main_price = (float)$val2['main_price'];
                    $discount_price = (float)$val2['discount_price'];
                    $percentage = ($main_price > 0 && $discount_price < $main_price) ? (($main_price - $discount_price) / $main_price) * 100 : 0;
                    //$percentage = number_format($percentage, 2); // Format to 2 decimal places
                ?>        
        <div class="col">
            <div class="mh-670 packages-box {{ $val2['most_popular'] != 0 ? 'package-active' : '' }}">
                <div class="tag">{{ ucfirst($val2['plan_name']) }}</div>
                <div class="dropdown packages">
                    <button class="btn m-c">
                        <h5>
                            <div class="mb-1">
                                @if($discount_price < $main_price)
                                    <div class="d-flex justify-content-center align-items-center mb-2">
                                        <div>
                                        <del>₹{{ number_format($val2['main_price']) }}</del>
                                        </div>
                                        @if($percentage > 0)
                                        <div class="off-button-wrapper ml-3">
                                            <div class="off-button">
                                    ...
                                    <div>
                                        <span><sup class="currency">₹</sup></span>
                                        <span>{{ number_format($val2['discount_price']) }}</span>
                                        <sub>/ mo</sub>
                                    </div>
                                @else
                                    <span><sup class="currency">₹</sup></span>
                                    <span>{{ number_format($val2['main_price']) }}</span>
                                    <sub>/ mo</sub>
                                @endif
                            </div>                           
                        </h5>
...
<div class="row row-cols-1 row-cols-sm-1 row-cols-md-4 row-cols-lg-4 row-cols-xxl-4 g-0">
    @if(!empty($data3))
        @foreach($data3 as $key3 => $val3) 
                <?php
                    $main_price = (float)$val3['main_price'];
                    $discount_price = (float)$val3['discount_price'];
                    $percentage = ($main_price > 0 && $discount_price < $main_price) ? (($main_price - $discount_price) / $main_price) * 100 : 0;
                    //$percentage = number_format($percentage, 2); // Format to 2 decimal places
                ?>        
        <div class="col">
            <div class="mh-670 packages-box {{ $val3['most_popular'] != 0 ? 'package-active' : '' }}">
                <div class="tag">{{ ucfirst($val3['plan_name']) }}</div>
                <div class="dropdown packages">
                    <button class="btn m-c">
                        <h5>
                            <div class="mb-1">
                                @if($discount_price < $main_price)
                                    <div class="d-flex justify-content-center align-items-center mb-2">
                                        <div>
                                        <del>₹{{ number_format($val3['main_price']) }}</del>
                                        </div>
                                        @if($percentage > 0)
                                        <div class="off-button-wrapper ml-3">
                                            <div class="off-button">
                                    ...
                                    <div>
                                        <span><sup class="currency">₹</sup></span>
                                        <span>{{ number_format($val3['discount_price']) }}</span>
                                        <sub>/ mo</sub>
                                    </div>
                                @else
                                    <span><sup class="currency">₹</sup></span>
                                    <span>{{ number_format($val3['main_price']) }}</span>
                                    <sub>/ mo</sub>
                                @endif
                            </div>                           
                        </h5>
...
<div class="text-center">
  <a href="#feature" class="btn-primary rounded btn-c mt-5">Compare Plans</a>
</div>
