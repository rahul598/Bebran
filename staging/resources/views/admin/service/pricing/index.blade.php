...
@endphp
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">{{ __('Pricing') }}</h1>
        </div>
        <div class="col-sm-6">
          ...
          <div class="card-body table-responsive">
            ...
            <table id="example2" class="table table-bordered table-hover dataTable">
              ...
              @endif
                  @endforeach
                  <td width="170">
                    <a href="{{ url(Admin_Prefix.'pricing/edit/'.$page->id) }}" title="Edit" class="btn btn-success"><i class="fa fa-fw fa-edit"></i></a>
                    <a href="{{ url(Admin_Prefix.'page/status/'.$page->id.'/'.$page->status) }}" title="Status" class="btn {{$page->status!=1?'btn-warning':'btn-info'}}"><i class="fa fa-fw fa-lightbulb"></i></a>
                    @if($page->id<0)
                    <a href="{{ url($page->slug) }}" title="View" target="_blank" class="btn btn-info"><i class="fa fa-fw fa-eye"></i></a>
                    @endif

                    @if($page->id != Pricing_Page_ID && $page->id != Development_Pricing_Page_ID)
