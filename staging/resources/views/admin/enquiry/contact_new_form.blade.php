...
<div class="content-header">
   <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">{{ __('Contact Page Data') }}</h1>
            <h1 class="m-0 text-dark d-none"><?php //echo "<pre>"; print_r(session()->get('selected_ids_contact')); ?></h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{url('admin')}}">{{ __('Home') }}</a></li>
              <li class="breadcrumb-item active">{{ __('Contact Page Form') }}</li>
            </ol>
          </div>
      </div>
   </div>
            ...
            <div class="card">
               <div class="card-header">
                  <div class="d-flex justify-content-start mb-4">
                    ...
                    <form action="{{ route('contact_csv',['id'=>7]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
...
$(document).ready(function() {
    ...
    $('#tableData').DataTable({
        ...
        "language": {
            ...
            $.ajax({
                url: '{{ route("getSessionIdsContact") }}',
                method: 'GET',
            ...
            function removeIdsFromSession(ids) {
                $.ajax({
                    url: "{{route('removeFromSessionContact')}}",
                    method: 'POST',
                    data: { ids: ids },
                    success: function(response) {
                        console.log(response.message);
            ...
            $('#deleteSelected').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('deleteFromDatabaseContact') }}",
                            method: 'POST',
                            data: {
