...
@endphp
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">{{ __('Case Studies') }} <a href="{{ url(Admin_Prefix.'case-study/add') }}" class="btn btn-primary">Add</a></h1>
        </div>
        <div class="col-sm-6">
        ...
        <table id="example2" class="table table-bordered table-hover dataTable">
          ...
          @if ($lists->count() > 0)
           ...
           <tr>
            ...
            @endforeach
            <td>
              <a href="{{ url(Admin_Prefix.'case-study/edit/'.$list->id) }}" title="Edit" class="btn btn-success"><i class="fa fa-fw fa-edit"></i></a>
              <a href="{{ url(Admin_Prefix.'page/status/'.$list->id.'/'.$list->status) }}" title="Status" class="btn {{$list->status!=1?'btn-warning':'btn-info'}}"><i class="fa fa-fw fa-lightbulb {{$list->status!=1?'inactive':''}}"></i></a>
              <!-- <a href="{{ url(Category_Prefix.$list->slug) }}" title="View" target="_blank" class="btn btn-info"><i class="fa fa-fw fa-eye"></i></a> -->
              @if(!in_array($list->id, Not_Deletable_Page_ID))
                <a href="{{ url(Admin_Prefix.'page/delete/'.$list->id) }}" data-confirm="" title="Delete" class="btn btn-danger"><i class="fa fa-fw fa-window-close"></i></a>
              @endif
            </td>
