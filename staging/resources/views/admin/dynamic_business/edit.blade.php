    ...
    <div class="row"> 
      ...
      <div class="col-md-12">
        <div class="card card-primary">
          ...
          <!-- /.card-header -->
          <!-- form start -->
          <form role="form" action="{{ url(Admin_Prefix.'business-service/update/') }}"  method="post" enctype="multipart/form-data" class="customValidate">

            @csrf

            <input type="hidden" name="id" value="{{$page->id}}">
            <input type="hidden" name="posttype" value="business-service">
            <input type="hidden" name="page_template" value="17">
              ...
              <div class="form-group row clearfix">
                ...
                <div class="col-sm-2">
                  <select name="service_parent_id" id="service_parent_id" class="form-control">
                    ...
                    <option value="{{$value->id}}" {{ $page->service_parent_id == $value->id?'selected':'' }}>{!! $value->page_name !!}</option>
                    @endforeach
                  </select>
                </div>
                @if($page->id>1)
                ...
                <div class="form-group row clearfix">
                    ...
                    <div class="col-sm-6">
                        <select name="price_widget" id="price_widget" class="form-control">
                            @php
                                $service_name1 = DB::table('pages')->where('id', $page->price_widget)->first();
                            @endphp
                            <option value="0">{{ $service_name1 ? $service_name1->page_name : '' }}</option>
                            @foreach($price_widget as $key => $value)
                                @php
                                    $service_name = DB::table('pages')->where('id', $value->service_type)->first();
                                @endphp
                                @if($value->service_type != $page->price_widget)
                                    <option value="{{ $value->service_type }}">{{ $service_name ? $service_name->page_name : '' }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
              ...
              <div class="form-group row clearfix">
                ...
                @foreach($page_extra as $val)
                      ...
                      <div class="col-sm-10">
                        ...
                        <div class="clearfix">
                          @if ($val->image && File::exists(public_path('uploads/' . $val->image)))
                            ...
                            {{-- <a href="{{ url('page-extra/content-delete/image/' . $val->id) }}" data-confirm="You want to delete this image!">
                      ...
                      <div class="col-sm-10">
                        ...
                        <div class="clearfix">
                          @if ($val->image2 && File::exists(public_path('uploads/'.$val->image2)))
                              ...
                              {{-- <a href="{{ url('page-extra/content-delete/image2/'.$val->id) }}" data-confirm="You want to delete this image!">
                                  <i class="fa fa-window-close"></i>
                              </a> --}}
                          @endif
                        </div>
                  ...
                  @if($val->section_type=="1" || $val->section_type==4 || $val->section_type==13 || $val->section_type==14 || $val->section_type==15 || $val->section_type==16 || $val->section_type==17 || $val->section_type==18 || $val->section_type==19 || $val->section_type==21 || $val->section_type==22)
                    <div class="form-group row clearfix">
                      <label class="col-sm-2 control-label">Content</label>
                      <div class="{{$val->section_type==4?'col-sm-8':'col-sm-8'}}">
                    ...
                    <div class="form-group row clearfix">
                      ...
                      <div class="{{$val->section_type==3?'col-sm-8':'col-sm-10'}}">
                        ...
                        <div class="clearfix">
                          @if($val->image && File::exists(public_path('uploads/'.$val->image)))
                              <img src="{{ asset('/uploads/'.$val->image) }}" style="margin: 10px 0 0 0;max-width: 200px;">
                              {{-- <a href="{{ url('page-extra/content-delete/image/'.$val->id) }}" data-confirm="You want to delete this slider!">
                                  <i class="fa fa-window-close"></i>
                              </a> --}}
                          @endif
                        </div>
