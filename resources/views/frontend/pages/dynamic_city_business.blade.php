@extends('layouts.app')
@section('more-css')
<link rel="preload" href="https://bebran.com/public/uploads/1726032038_digital-mark.jpg" as="image">
...
.bg_acc_new h2 strong, .bg_acc_new h3 strong, .bg_acc_new h2 span{
    background: #033355;
    padding: 0 10px;
    border-radius: 10px;
}
...
.service-inner-banner {
    background: url('low-quality-placeholder.jpg') no-repeat center center;
...
@foreach ($extra_data as $val)
    ...
    @endswitch
@endforeach
    <div class="service-inner-banner digital-marketing" data-page-id="{{$page_id}}" style="background: url({{ (!empty($first_image) && file_exists(public_img_path($first_image))) ? asset('uploads/' . $first_image) : 'Image not found' }});">
        
         <div class="container">
            <div class="service-inner-body d-md-flex justify-content-between align-items-center">
               <div class="digitaltext">
                  <h1 class="text-white">@if(!empty($first_title)) {!! $first_title !!}@endif</h1>
        ...
        <div class="innerbanner-area">
            <nav class="breadcrumb bg_acc_new">
              <div class="container">
                ...
                <span class="breadcrumb-item active"><span class="span">{{ $business_name }}</span></span>
              </div>
            </nav>
          </div>
         <div class="container">
            <div class="row">
               ...
               <div class="col-lg-6 col-md-6">
                  <div class="about-area-two-contain citytextarea bg_acc_new">
                     ...
                     <div class="citytext">
                      @foreach ($extra_data as $key=>$val)
                        @if($val->type == 4 )
                            ...
                            <div class="citybody">
                                <div>@if(!empty($val->title)) {!! $val->title !!}@endif
                                </div>
                            </div>
                            </div>
                            @endif
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-6 seoarea d-flex align-items-stretch">
                        <div class="seobox cityseoboxin">
                            <div class="cityseoiconarea">
                                ...
                                <div class="iconbody">
                                    <div class="seoicon_h4">{{ $val->title }}</div>
                                </div>
                            </div>
                              {!! $val->body !!}
      ...
      <div class="professional businesssection innerbusiness bg_acc_new">
         <div class="container">
            <div class="headingbox text-center">
                 ...
                 @php $countNum = 0; @endphp
                    ...
                    @if($val->type == 8 )
                        ...
                        <button class="nav-link {{ $countNum ==1 ? 'active' : '' }}" data-bs-toggle="pill" data-bs-target="#tab{{ $key }}">
                            ...
                            <div class="h4_innerMission">{!! $val->title !!}</div>
      ...
      <div class="helpyou approacharea bg_acc_new">
         <div class="container">
            <div class="headingbox text-center">
      ...
      <div class="tooluse aboutdigital aboutcity cityaboutarea bg_acc_new">
         <div class="container">
            <div class="headingbox text-center d-flex justify-content-center">
                <h2>@if(!empty($eleven_title)) {!! $eleven_title !!}@endif</h2>
            </div>
            <div class="row">
                @foreach ($extra_data as $key=>$val)
                    @if($val->type == 12 )
                    <div class="col d-flex align-items-stretch mobileMargin">
                            ...
                            <div class="card-body d-flex align-items-center justify-content-center">
                                ...
                                <div class="seoiconarea">
                                    ...
                                    <div class="digitalbody">
                                        <div class="about-service-box_h4 ">{{ $val->title }}</div>
                                    </div>
                                </div>
                                {!! $val->body !!}
                                </div>
      ...
      <div class="whyus tools-area bg_acc_new">
         <div class="container">
                ...
                <div class="row">
                    ...
                    <div class="bolgbox">
                        ...
                        <div class="btxt">
                            ...
                            <div class="stxt">
                            <div style="font-size: 14px; margin-bottom: 5px; color: #191919; font-weight: 600;">@if(!empty($val->page_title)){{ $val->page_title }}@endif</div>
                            <div style="font-size: 11px; font-weight: 500;">@if(!empty($val->bannertext)){{ $val->bannertext }}@endif</div>
                            </div>
                        </div>
                        </div>
                        <div class="boltxt">
...
$(document).ready(function() {
    $('.duration').on('click', function() {  
        ...
        //  get image
    $(document).ready(function() {
    // Function to load background image via AJAX POST after a delay
    function loadBackgroundImage(pageId, targetElement) {
        setTimeout(function() {
            $.ajax({
                url: '{{route("imageGet")}}',  // The server endpoint to get the image URL
                method: 'POST',
                data: {
                    page_id: pageId,  // Send the page ID in the request
                    _token: '{{ csrf_token() }}'  // Include CSRF token if you're using Laravel
                },
                success: function(response) {
                    // Assuming the server returns a JSON response with the image URL
                    if (response.image_url) {
                        $(targetElement).css('background-image', 'url(' + response.image_url + ')');
                    }
                },
        ...
        }, 5000);  // Delay in milliseconds (200ms or adjust as needed)
    }

    // Use IntersectionObserver to lazy-load the background image
    var observer = new IntersectionObserver(function(entries) {
        entries.forEach(function(entry) {
            if (entry.isIntersecting) {
                var pageId = $(entry.target).data('page-id');
                loadBackgroundImage(pageId, entry.target);
                observer.unobserve(entry.target);  // Stop observing once loaded
            }
        });
    });

    // Observe each banner for lazy loading
    $('.service-inner-banner').each(function() {
