...
.businesssection .headingbox h2::after {
    content: "";
    position: absolute;
    left: 0;
    bottom: -20px;
...
.tooluse .headingbox h2:after, .helpyou .headingbox h2:after, .professional .headingbox h2:after {
...
.tooluse .headingbox h2::before, .helpyou .headingbox h2::before, .businesssection .headingbox h2::before {
    content: "";
    position: absolute;
    left: 0;
    bottom: -26px;
...
.tooluse .headingbox h2:before, .helpyou .headingbox h2:before, .professional .headingbox h2:before {
    /*background: rgb(255 255 255 / 72%);*/
}
...
.tooluse .headingbox h2, .helpyou .headingbox h2, .businesssection .headingbox h2 {
    color:#00003a !important;
    position: relative;
    margin-bottom: 70px;
}
.tooluse .headingbox h2, .helpyou .headingbox h2, .professional .headingbox h2 {
    /*color: #fff;*/
}
.headingbox h2 {
    font-size: 34px;
...
.tooluse .headingbox h2 strong,.helpyou .headingbox h2 strong, .professional .headingbox h2 strong {
    font-weight: 700;
    color: #ffc145;
}
...
.seoclientsay-area h2, .faqbg .headingbox h2 {
    color: #fff;
}
.seoclientsay-area h2:before, .faqbg .headingbox h2:before {
    background: #fff;
}
.seoclientsay-area h2:after, .faqbg .headingbox h2:after {
    background: #fff;
}
...
<div class="service-inner-banner digital-marketing city-banner" style="background: url({{ (!empty($first_image) && file_exists(public_img_path($first_image))) ? asset('uploads/' . $first_image) : 'Image not found' }});">
    <div class="container">
       <div class="service-inner-body d-md-flex justify-content-between align-items-center">
          <div class="digitaltext">
             ...
             <p class="first_color">@if(!empty($first_sub_title)) {!! $first_sub_title !!}@endif</p>
          </div>
    ...
    <div class="innerbanner-area">
        <nav class="breadcrumb">
          <div class="container">
            ...
            <span class="breadcrumb-item">{{ $city_name }}</span>
          ...
          <div class="col-lg-6 col-md-6">
             <figure class="about-area-two-img">
                @if(!empty($third_image))
                    ...
                    <img src="{{ asset('uploads/' . $third_image) }}" alt="{{ getImageNameAlt($third_image) }}" width="485" height="475">
                    @else
                        Image not found
                    @endif
                @endif
                ...
                <div class="citytext">
                    ...
                    @php $increment++ ;@endphp
                        <div class="city-area d-flex align-items-center">
                            <div class="cityicon">{{ sprintf('%02d', $increment) }}</div>
                            <div class="citybody">
 ...
 <div class="ourservice cityservice">
    <div class="container">
       <div class="headingbox text-center">
            <h2 class="fz-35 bottom_line">
                @if(!empty($fifth_title)) 
                    {!! $fifth_title !!}
                @endif
            </h2>
            <h2 class="fz-35 second_color">
                @if(!empty($fifth_sub_title)) 
                    {!! $fifth_sub_title !!}
                @endif
            </h2>
        </div>
          ...
          @foreach ($extra_data as $key=>$val)
            @if($val->type == 6 )
                <div class="col-lg-4 col-md-6 col-sm-6 seoarea d-flex align-items-stretch">
                    <div class="seobox seoboxin cityboxin">
 ...
 <div class="professional businesssection">
    <div class="container">
       <div class="citybusines">
          <div class="headingbox text-center">
             <h2 class=" ">@if(!empty($seventh_title)) {!! $seventh_title !!}@endif <br>
 ...
 <div class="helpyou approachsection cityhelpbody">
    <div class="container">
       <div class="headingbox text-center">
                     ...
                     @foreach ($extra_data as $key=>$val)
                        ...
                        <div class="tab-pane fade {{ $counteIncrease == 1 ? 'show active' : '' }}" id="SERVICES{{ $key }}" role="tabpanel">
                            ...
                            <div class="marketing-contain">
                                {!! $val->body !!}
                                <a href="{{ $val->btn_url }}" class="btn-primary">{{ $val->btn_text }}</a>
                            </div>
                        </div>
                     @endif
                   @endforeach
 ...
 <div class="tooluse aboutdigital aboutcity">
    <div class="container">
       <div class="headingbox text-center">
 ...
 <div class="tooluse partnersection d-none">
    <div class="container">
       <div class="headingbox text-center">
          <h3>@if(!empty($seventeen_title)) {!! $seventeen_title !!}@endif</h3>
       </div>
       <div class="owl-carousel tool-carousel12">
        @foreach ($extra_data as $key=>$val)
            @if($val->type == 18 )
                <figure>
                    @if(!empty($val->image) && file_exists(public_img_path($val->image))) <img src="{{ asset('uploads/' . $val->image) }}" alt="{{ getImageNameAlt($val->image) }}"> @else Image not found @endif
...
<!--------------ourclient video ------------------> 
 ...
 <div class="review">
    <div class="container">
       <div class="headingbox text-center ">
          <h3>@if(!empty($twenty_three_title)) {!! $twenty_three_title !!}@endif</h3>
       </div>
