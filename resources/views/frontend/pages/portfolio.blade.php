    ...
    <div class="container">
        <div class="casestudytab">
            <nav class="nav nav-tabs justify-content-center" id="nav-tab1" role="tablist">
                <a class="nav-link active" data-bs-toggle="tab" href="#allcasestudies" role="tab" aria-selected="true">All Portfolio</a>
                @php $dup_check_category = []; @endphp
                @foreach($portfolioCategoryData as $key => $category)
                  @foreach($category as $study)
                    @if(!in_array($study->slug, $dup_check_category))
                      <a class="nav-link" data-bs-toggle="tab" href="#{{ $study->slug }}" role="tab" aria-selected="false" tabindex="-1">{{ $study->name }}</a>
                    @endif
                    @php $dup_check_category[] = $study->slug; @endphp
                  @endforeach
                @endforeach
            </nav>
            ...
            <div class="tab-content" id="nav-tabContent1">
                <div class="tab-pane fade active show" id="allcasestudies" role="tabpanel">
                    ...
                    @if(!empty($study->bannerimage) && file_exists(public_img_path($study->bannerimage))) <img src="{{ asset('uploads/' . $study->bannerimage) }}" alt="{{ getImageNameAlt($study->bannerimage) }}"> @else Image not found @endif
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    {!! $study->body !!}
                                                    <a href="{{ url('portfolio/'.$study->page_slug) }}" class="read">Read More</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                {{-- @php echo '<pre>'; print_r($portfolioCategoryData); die; @endphp --}}
                @foreach($portfolioCategoryData as $key => $category)
