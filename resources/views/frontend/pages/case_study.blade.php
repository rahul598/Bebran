@extends('layouts.app')
<style>
.casestudytab .portfoliosection{
    padding:0 !important;
}
.casestudytab .seoLandingBox{
...
<div class="service-inner-banner digital-marketing case-study-banner" style="background: url({{ (!empty($first_image) && file_exists(public_img_path($first_image))) ? asset('uploads/' . $first_image) : 'Image not found' }});">
  ...
  <div class="packages-area seosection casestudysection">
    ...
    <a class="nav-link active" data-bs-toggle="tab" href="#allcasestudies" role="tab" aria-selected="true">All case studies</a>

    @php
        // Initialize an empty array to track unique categories
        $uniqueCategories = [];
    @endphp

    @foreach($caseStudiesCategoryData as $category)
        @foreach($category as $study)
            @if (!in_array($study->category_name, $uniqueCategories))
                <a class="nav-link" data-bs-toggle="tab" href="#{{ $study->slug }}" role="tab" aria-selected="false">{{ $study->category_name }}</a>

                @php 
                    $uniqueCategories[] = $study->category_name;
                @endphp
            @endif
        @endforeach
    @endforeach
    ...
    <div class="tab-content" id="nav-tabContent1">
                                ...
                                <div class="portfoliosection portfoliobox">
                                    <div class="card seoLandingBox">
                                        <div class="card-img d-flex">
                                            @if(!empty($study->bannerimage) && file_exists(public_img_path($study->bannerimage))) 
                                                <img src="{{ asset('uploads/' . $study->bannerimage) }}" alt="{{ getImageNameAlt($study->bannerimage) }}">
                                            @else
                                                Image not found
                                            @endif
                                        </div>
                                        <div class="card-body">
                                            <div class="portfolioicon">
                                                @if(!empty($study->image2) && file_exists(public_img_path($study->image2))) 
                                                    <img src="{{ asset('uploads/' . $study->image2) }}" alt="{{ getImageNameAlt($study->image2) }}"> 
                                                @else 
                                                    Image not found 
                                                @endif
                                            </div>
                                            <h6>{{ $study->page_name }}</h6>
                                            {!! $study->body !!}
                                            <a href="{{ url('case-study/'.$study->page_slug) }}" class="read">Read More</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        @php $dup_check_category = []; @endphp
        @foreach($caseStudiesCategoryData as $category)
            @if(!empty($category[0]) && !in_array($category[0]->slug, $dup_check_category))
                <div class="tab-pane fade" id="{{ $category[0]->slug }}" role="tabpanel">
                    <div class="headingbox text-center">
                        <h3>{{ $category[0]->category_name }} <strong>@if(!empty($third_title)) {!! $third_title !!}@endif</strong></h3>
                    </div>
                    <div class="row">
                        @foreach($category as $study)
                            <div class="col-lg-4 col-md-6">
                ...
                @php $dup_check_category[] = $category[0]->slug; @endphp
            @endif
        @endforeach
    </div>
</div>
  ...
  <div class="Increment-area talknumber talksection d-none" style="background: url({{ asset('frontend/images/detilsbg.webp') }});">
    <div class="container">
      <div class="headingbox text-center">
        <h3>@if(!empty($fourth_title)) {!! $fourth_title !!}@endif</h3>
      </div>
      <div class="incrarea">
