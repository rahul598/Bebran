...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">{{ __('Packages Type') }} <a href="{{ url(Admin_Prefix.'package-type/add') }}" class="btn btn-primary">Add</a></h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{url('admin')}}">{{ __('Home') }}</a></li>
          <li class="breadcrumb-item active">{{ __('Packages Type') }}</li>
        </ol>
      </div>
    </div>
  </div>
          ...
          @if ($lists->count() > 0)
           ...
           <tr>
            @foreach($column_array as $key => $value)
              ...
              @elseif ($key=='page_id')              
                <td>@if(!empty($list->page->page_name)){!! $list->page->page_name !!}@endif</td>
            ...
            @endforeach
            <td>
              <a href="{{ url(Admin_Prefix.'package-type/edit/'.$list->id) }}" title="Edit" class="btn btn-success"><i class="fa fa-fw fa-edit"></i></a>
              <a href="{{ url(Admin_Prefix.'package-type/status/'.$list->id.'/'.$list->status) }}" title="Status" class="btn {{$list->status!=1?'btn-warning':'btn-info'}}"><i class="fa fa-fw fa-lightbulb {{$list->status!=1?'inactive':''}}"></i></a>
              <!-- <a href="{{ url(Category_Prefix.$list->slug) }}" title="View" target="_blank" class="btn btn-info"><i class="fa fa-fw fa-eye"></i></a> -->
              <a href="{{ url(Admin_Prefix.'package-type/delete/'.$list->id) }}" data-confirm="" title="Delete" class="btn btn-danger"><i class="fa fa-fw fa-window-close"></i></a>
            </td>
