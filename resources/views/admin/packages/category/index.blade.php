...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">{{ __('Packages Category') }} <a href="{{ url(Admin_Prefix.'package-category/add') }}" class="btn btn-primary">Add</a></h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{url('admin')}}">{{ __('Home') }}</a></li>
          <li class="breadcrumb-item active">{{ __('Packages Category') }}</li>
        </ol>
      </div>
    </div>
  </div>
        ...
        <table id="example2" class="table table-bordered table-hover dataTable">
          ...
          @if ($lists->count() > 0)
           ...
           <tr>
            ...
            @endforeach
            <td>
              <a href="{{ url(Admin_Prefix.'package-category/edit/'.$list->id) }}" title="Edit" class="btn btn-success"><i class="fa fa-fw fa-edit"></i></a>
              <a href="{{ url(Admin_Prefix.'package-category/status/'.$list->id.'/'.$list->status) }}" title="Status" class="btn {{$list->status!=1?'btn-warning':'btn-info'}}"><i class="fa fa-fw fa-lightbulb {{$list->status!=1?'inactive':''}}"></i></a>
              <!-- <a href="{{ url(Category_Prefix.$list->slug) }}" title="View" target="_blank" class="btn btn-info"><i class="fa fa-fw fa-eye"></i></a> -->
              <a href="{{ url(Admin_Prefix.'package-category/delete/'.$list->id) }}" data-confirm="" title="Delete" class="btn btn-danger"><i class="fa fa-fw fa-window-close"></i></a>
            </td>
