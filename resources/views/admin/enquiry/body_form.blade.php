...
<div class="content-header">
   <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">{{ __('Body Form Data') }}</h1>
            <h1 class="m-0 text-dark d-none"><?php //echo "<pre>"; print_r(session()->get('selected_ids_body')); ?></h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{url('admin')}}">{{ __('Home') }}</a></li>
              <li class="breadcrumb-item active">{{ __('Body Form') }}</li>
            </ol>
          </div>
      </div>
   </div>
...
$(document).ready(function () {
    ...
    // Fetch session IDs on page load and set checkboxes 
            ...
            function removeIdsFromSession(ids) {
                $.ajax({
                    url: "{{route('removeFromSessionbody')}}",
                    method: 'POST',
                    data: { ids: ids },
                    success: function(response) {
                        console.log(response.message);
            ...
            $('#deleteSelected').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('deleteFromDatabasebody') }}",
                            method: 'POST',
                            data: {
