...
@endphp
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">{{ __('Edit Case Studies') }}</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{url('admin')}}">{{ __('Home') }}</a></li>
            <li class="breadcrumb-item active">{{ __('Edit Case Studies') }}</li>
          </ol>
        </div>
      </div>
    </div>
      ...
      <div class="col-md-12">
        ...
        <div class="card card-primary">
          ...
          <form role="form" action="{{ url(Admin_Prefix.'page/update/') }}"  method="post" enctype="multipart/form-data" class="customValidate">
            ...
            <input type="hidden" name="posttype" value="case-study">
              ...
              <div class="form-group row clearfix">
                ...
                <div class="col-sm-10">
                  <select name="category_id" class="form-control select2" data-placeholder="Select">
                    ...
                    @foreach($case_studies_categories as $key => $value)
                    <option value="{!! $value->id !!}" @if($value->id == $page->case_study_category) selected @endif>{!! $value->name !!}</option>
                    @endforeach
                  </select>
                </div>
              </div>
