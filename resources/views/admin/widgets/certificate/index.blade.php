    ...
    <div class="row"> 
      ...
      <div class="col-md-12">
        ...
        <div class="px-3 py-4 border-bottom-2">
            <form action="{{ route('updateTitleContent', $component_content->id) }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="title" class="form-label">Title</label>
                    <input type="text" class="form-control" id="title" name="title" value="{{ $component_content->title }}">
                </div>
                <div class="mb-3">
                    <label for="content_body" class="form-label">Text Body</label>
                    <textarea class="form-control summernote" id="content_body" name="content_body">{{ $component_content->body }}</textarea>
                </div>
                <button type="submit" class="btn btn-primary">Update Digital Certified Content</button>
            </form>
        ...
        <table id="example2" class="table table-bordered table-hover dataTable">
          ...
          <?php $row = 1 ; ?>
           ...
           <tr>
                ...
                <td>
                   <textarea class="form-control" name="embaded_code" form="form_{{ $list->id }}">{{ $list->embaded_code }}</textarea>
                </td> 
               <td>
                ...
                <td>
                        <form id="form_{{ $list->id }}" action="{{ route('updateDeleteToggleCertified', ['id' => $list->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
...
<!-----------------model----------------->
 <form class="form-control" action="{{ route('storeFormDataCertified') }}" method="POST" enctype="multipart/form-data">
     @csrf
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div> OR </div>
            <div class="mb-3">
                <label for="exampleInputEmail2" class="form-label">Embedded code</label>
